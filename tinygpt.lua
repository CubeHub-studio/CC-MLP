...
local function mat(r,c) local a={} for i=1,r do a[i]={}; for j=1,c do a[i][j]=rand() end end return a end
local function vec(n) local a={}; for i=1,n do a[i]=rand() end return a end
local function dot(a,b) local s=0 for i=1,#a do s=s+a[i]*b[i] end return s end
local function softmax(a)
 local m=-math.huge; for i=1,#a do if a[i]>m then m=a[i] end end
 local e,sum={},0
 for i=1,#a do e[i]=math.exp(math.max(-60,math.min(60,a[i]-m))); sum=sum+e[i] end
 for i=1,#a do e[i]=e[i]/sum end
 return e
end
local function outer_add(M,a,b,scale)
 for i=1,#a do for j=1,#b do M[i][j]=M[i][j]+scale*a[i]*b[j] end end
end
local function zeros_like(M) local r={}; for i=1,#M do r[i]=zeros(#M[i]) end return r end
local function addv(a,b) local r={}; for i=1,#a do r[i]=a[i]+b[i] end return r end
local function mv(M,x,b)
 local y={}
 for i=1,#M do local s=b and b[i] or 0; for j=1,#x do s=s+M[i][j]*x[j] end y[i]=s end
 return y
end
local function mtv(M,x)
 local y=zeros(#M[1]); for i=1,#M do for j=1,#M[i] do y[j]=y[j]+M[i][j]*x[i] end end return y
end
...
function GPT:forward(tokens)
 ...
 for t=1,L do x[t]=self.E[tokens[t]+1]; q[t]=mv(self.Wq,x[t]); k[t]=mv(self.Wk,x[t]); v[t]=mv(self.Wv,x[t]) end
 local scale=math.sqrt(self.d)
 for t=1,L do
  local scores={}; for j=1,t do scores[j]=dot(q[t],k[j])/scale end; a[t]=softmax(scores)
  local z=zeros(self.d); for j=1,t do for c=1,self.d do z[c]=z[c]+a[t][j]*v[j][c] end end
  h[t]=addv(x[t],z)
 end
 local fpre,ffout,y={},{},{}
 for t=1,L do
  fpre[t]=mv(self.W1,h[t],self.b1); ffout[t]=zeros(self.ff)
  for j=1,self.ff do ffout[t][j]=math.max(0,fpre[t][j]) end
  y[t]=addv(h[t],mv(self.W2,ffout[t],self.b2))
 end
 local probs={}; for t=1,L do probs[t]=softmax(mv(self.O,y[t],self.bo)) end
...
function GPT:train(tokens,targets)
 ...
 for t=1,L do
  local p=c.probs[t]; local target=targets[t]+1; loss=loss-math.log(math.max(p[target],1e-12))
  local go=zeros(self.vocab); for vtx=1,self.vocab do go[vtx]=p[vtx]-(vtx==target and 1 or 0) end
  outer_add(g.O,go,c.y[t],1); for vtx=1,self.vocab do g.bo[vtx]=g.bo[vtx]+go[vtx] end
  gy[t]=mtv(self.O,go)
 end
 local gh={}
 for t=1,L do
  gh[t]=addv(gy[t],gy[t]); local gf=mtv(self.W2,gy[t])
  for j=1,ff do if c.fpre[t][j]<=0 then gf[j]=0 end; g.b1[j]=g.b1[j]+gf[j] end
  outer_add(g.W2,gy[t],c.ff[t],1); gh[t]=addv(gh[t],mtv(self.W1,gf))
  for j=1,d do g.b2[j]=g.b2[j]+gy[t][j] end; outer_add(g.W1,gf,c.h[t],1)
 end
 ...
 for t=1,L do
  ...
  for j=1,t do
   ...
   for r=1,d do
    local gv=w*gz[r]
    for cc=1,d do g.Wv[r][cc]=g.Wv[r][cc]+gv*c.x[j][cc] end
    for cc=1,d do gx[j][cc]=gx[j][cc]+gv*self.Wv[r][cc] end
   end
   ga[j]=dot(gz,c.v[j])
  end
  ...
  for j=1,t do
   ...
   for r=1,d do gq[r]=gs*c.k[j][r]; gk[r]=gs*c.q[t][r] end
 ...
 local function apply(M,G) for i=1,#M do for j=1,#M[i] do M[i][j]=M[i][j]-lr*math.max(-5,math.min(5,G[i][j])) end end end
 local function applyv(M,G) for i=1,#M do M[i]=M[i]-lr*math.max(-5,math.min(5,G[i])) end end
...
function GPT:train_text(tokens,epochs,seq_len)
 ...
 for ep=1,epochs do
  ...
  for i=1,#tokens-seq_len do
   local x,y={},{}; for j=1,seq_len do x[j]=tokens[i+j-1]; y[j]=tokens[i+j] end
